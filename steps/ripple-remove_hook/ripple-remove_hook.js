...
module.exports = async function (workflowId, stepName, step, log, callback) {
  // Validate parameters
  if (step.parameters.account_info.Sequence == undefined) {
    step.status = 'error';
    step.message = `[${workflowId}] [${stepName}]: ${metadata.name}@${metadata.version}, invalid parameters requirement.`
    callback(step);
  }
  ...
  try {
    ...
    // log.debug(`[${workflowId}] [${stepName}]: base_drops ${JSON.stringify(base_drops)}`);
